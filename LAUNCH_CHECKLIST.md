...
pnpm --filter @lore-ai/core    pack --pack-destination /tmp/lore-tarballs
pnpm --filter @lore-ai/parser  pack --pack-destination /tmp/lore-tarballs
...
pnpm --filter @lore-ai/core    publish --dry-run --no-git-checks
pnpm --filter @lore-ai/parser  publish --dry-run --no-git-checks
```
...
> 본 프로젝트는 `lore-ai` (unscoped) + `@lore-ai/core`, `@lore-ai/parser` (scoped) 혼합. org 생성 필요.
...
npm view @lore-ai/core version     # 0.1.0
npm view @lore-ai/parser version   # 0.1.0
...
- 태그: `lore-ai@0.1.0`, `@lore-ai/core@0.1.0`, `@lore-ai/parser@0.1.0`
...
scoped 패키지 (`@lore-ai/*`) 가 publishConfig 없이 publish 시도된 경우. 확인:
...
# 2. npm 이름
npm view lore-ai @lore-ai/core @lore-ai/parser

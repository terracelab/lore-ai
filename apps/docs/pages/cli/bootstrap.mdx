...
3. **그 응답을 받은 직후** AI 에게 _"위 diff 를 실제로 두 파일에 적용해줘"_ 한 번 더 지시 — Claude Code 는 Edit/Write 툴로 직접 파일을 고침
...
API key 가 없거나 즉시 결과가 필요할 때 — 폴더 이름만으로 도메인 후보 + subdomains 초안. 결과는 거칠지만 _완전히 빈 시드_ 보다 빠른 시작.
...
- `config/`, `core/` 같은 _인프라성_ 이름 → 명시적으로 제외 (도메인이 아니라 설정/공통)
